...
| Accuracy math | [`GetTotalAccuracy`](../../../src/battle_util.c) | `AB_HELPER_SUPPLIED_TARGET_STATE` | Implemented / clarify contract | Live accuracy math uses active No Guard, Compound Eyes, Hustle, Wonder Skin, Tangled Feet, Sand Veil, Snow Cloak, and Victory Star membership where battle state is authoritative. The supplied-ability `GetTotalAccuracy` path remains an explicit caller contract for AI and damage-context-style prediction. |
...
| Move-end iteration | [`SharedPower_GetNextMoveEndAbility`](../../../src/battle_util.c) | `AB_EFFECTIVE_SHARED_ITERATION_EVENT` | Stay / clarify callbacks | Iteration is shared-aware and remains the intended event dispatcher. Callback-specific hardcoded target/global-state questions stay in the clarify-only queue. |
...
| Move effect application | [`SetMoveEffect`](../../../src/battle_script_commands.c) | `AB_CLARIFY_HARDCODED_TARGET_STATE` | Implemented buckets / clarify remainder | Selected buckets are implemented: Flame Burst Magic Guard, flinch Inner Focus, and standard confusion-related Own Tempo gates. The helper still mixes explicit args with globals for other effects, so remaining cases are clarify-only until each source/target contract is reviewed. |
...
| Stat buffs | [`ChangeStatBuffs`](../../../src/battle_script_commands.c) | `AB_EFFECTIVE_SHARED_MEMBERSHIP` | Mostly implemented / clarify attribution | Active stat-loss prevention and punish abilities are handled through Shared Power-aware checks. Flower Veil source attribution remains a popup/record clarification question, not an open migrate row. |
...
| End-turn passive damage | weather damage, poison, burn, nightmare, curse, leech seed, Yawn | `AB_EFFECTIVE_SHARED_MEMBERSHIP` | Implemented buckets / clarify remainder | Shareable residual-damage and prevention buckets such as Magic Guard, Poison Heal, Liquid Ooze direct drain, and Early Bird are implemented. Leech Seed Liquid Ooze and Yawn sleep-prevention ownership remain clarify-only hardcoded-state questions. |
...
| AI damage usability | `IsDamageMoveUnusable`, `AI_CalcDamage` | `AB_AI_CACHED_PREDICTION` | Clarify / defer | AI damage prediction has many active-aware wrappers, but full `DamageContext` versus live-pool semantics remain tied to the broader damage-context policy. Do not migrate further until that contract is decided. |
| AI known ability | `AI_DecideKnownAbilityForTurn`, switch prediction, switch items | `AB_AI_CACHED_PREDICTION` | Implemented / deferred tail | AI should reason over the current Shared Power pool through active-aware adapters. Shareable scoring and switch paths are migrated, including status blockers, move blockers, Klutz item checks and hold-effect prediction, Sticky Hold item-loss checks, hazard Magic Bounce, Wonder Guard smart switching and bad-move scoring, active secondary-damage prediction for Magic Guard and Poison Heal, direct-drain Liquid Ooze prediction, Strength Sap Contrary prediction, generic stat-up Contrary prediction, Hone Claws, Stockpile, Shell Smash, Belly Drum and Fillet Away bad-move Contrary prediction, Gear Up and Magnetic Flux Plus/Minus prediction, Utility Umbrella weather-ability scoring for Solar Power and Dry Skin, Wide Guard partner-damage Telepathy prediction, partner-crit Anger Point prediction, Fell Stinger, Rototiller ally and foe Contrary prediction, self/target additional-effect Contrary prediction, Defog target Contrary prediction, Swagger and Flatter Contrary target scoring, Merciless poison-score prediction, Serene Grace confusion-score prediction, Leech Seed Magic Guard prediction, Sturdy endure prediction, Recycle Ripen prediction, multi-hit Rocky Helmet and low-accuracy crash-recoil Magic Guard prediction, Rest fast-recovery prediction for Shed Skin, Early Bird, and Hydration, Focus Energy and Laser Focus crit-setup prediction for Super Luck and Sniper, Foresight Scrappy and Mind's Eye prediction, Substitute/Shed Tail Infiltrator prediction, Magnitude Levitate prediction, Lock-On No Guard prediction, Laser Focus crit-blocker prediction, Black Sludge Trick Magic Guard prediction, Prankster-into-Dark status prediction, guaranteed flinch prediction for Inner Focus and Shield Dust, active Heal Bell Soundproof prediction, active Comatose sleep-state prediction, active Overcoat powder prediction, active Perish Song Soundproof prediction, active Roar Soundproof and Suction Cups prediction, active Electric-absorption self-benefit prediction for Ion Deluge and Electrify, active stat-loss blocker and stat-drop-punish prediction for Spicy Extract and stat-drop scoring, active weather/terrain benefit prediction for shareable ability heuristics, and active ally-targeting HP-aware prediction for absorption abilities. The remaining exact direct cached-ability reads are classified below as native-only or clarify-deferred rather than open migrate work. |

## AI Native / Deferred Tail

The remaining direct `aiData->abilities[]` / `gAiLogicData->abilities[]` comparisons after the Shared Power AI migration pass are intentionally not migrated in this sweep:

| Callsite | Classification | Default until clarified | Rationale |
| --- | --- | --- | --- |
| `src/battle_ai_util.c` Shadow Tag self-exemption in `IsBattlerTrapped` and `src/battle_ai_main.c` Run Away roaming flee check | `AB_RECORD_ATTRIBUTION` | Defer | The Run Away/trapping row is already clarify-deferred. Changing the AI first would preempt unresolved policy on donated trap abilities, Shadow Tag's self-exemption, and trap source/display attribution. |
| `src/battle_ai_util.c` Opportunist stat-setup guard | `AB_AI_CACHED_PREDICTION`, `AB_MUTATION_NATIVE_SLOT` | Defer | Live Opportunist stat-copy activation still checks native `GetBattlerAbility` when marking and executing copied stat changes. Do not make AI predict pooled Opportunist before live behavior is explicitly migrated. |
| `src/battle_ai_main.c` Liquid Ooze checks in Leech Seed scoring | `AB_CLARIFY_HARDCODED_TARGET_STATE` | Defer | Direct drain move prediction is migrated, but Leech Seed remains under the end-turn ownership clarification: decide whether Liquid Ooze belongs to the seeded victim, the drained target, or another effective source before changing AI. |
| `src/battle_ai_main.c` Receiver scoring when an ally is expected to faint | `AB_MUTATION_NATIVE_SLOT` | Stay native | Receiver and Power of Alchemy copy a native ability slot into another native slot. The audit explicitly keeps copy/mutation effects native-only. |
| `src/battle_ai_main.c` Truant skill-swap and switch heuristics | `AB_NATIVE_SUPPRESSED_SINGLE` | Defer / stay native | Live Truant move cancellation uses native `GetBattlerAbility`, and switch-in/end-turn bookkeeping also manipulates a single native `truantCounter`. Do not predict pooled Truant until the live Truant policy is explicitly changed. |
| `src/battle_ai_main.c` Natural Cure smart-switch heuristic | `AB_NATIVE_SUPPRESSED_SINGLE` | Defer / stay native | Live switch-out curing uses native `GetBattlerAbility`. Do not make AI switch for pooled Natural Cure before live switch-out behavior is changed. |
| `src/battle_ai_main.c` Stance Change King's Shield Aegislash scoring | `AB_NATIVE_ONLY_FORM_OR_SPECIES_GATE` | Stay native | The heuristic is explicitly species/form-gated to Aegislash Blade and must not use donated Stance Change. |

## Remaining Clarify-Only Queue

The migration sweep intentionally leaves these rows as policy questions instead of code changes. They are not open migrate rows until their owner/source/attribution contract is decided.

| Topic | Default until clarified | Next question |
| --- | --- | --- |
| Damage context and type effectiveness | Defer broader migration | When should `DamageContext.abilityAtk/Def` be authoritative, and when may helpers re-query live Shared Power membership despite Mold Breaker, Unaware-style suppression, or selected ability context? |
| Contact helper contract | Defer broader migration | Should callers request raw contact, native-contact-with-supplied-ability, or active Shared Power-modified contact? |
| Status helper contract | Defer broader migration | Is `CanSetNonVolatileStatus` a pure predicate using supplied ability state, or should it resolve source, target, popup, and side-blocker attribution itself? |
| Move-end target and attacker callbacks | Defer per callback | For each callback, which battler owns the pooled ability, which battler is affected, and which native mutation cases must stay native-only? |
| `SetMoveEffect` remaining effects | Defer per effect | For each remaining effect branch, should source and target be the explicit args, `gBattlerAttacker`, `gBattlerTarget`, or `gEffectBattler`? |
| `BS_SetLastUsedAbility` | Defer | Which script callers require native ability identity, and which should display the selected active Shared Power ability? |
| Neutralizing Gas | Defer | Which Neutralizing Gas paths are special native-slot gas behavior, and which, if any, should interact with pooled ability membership? |
| Order tiebreakers | Defer | Should donated Stall, Mycelium Might, Quick Draw, and Custap-like effects affect within-priority ordering, and how should multiple donated order effects compose? |
| Dynamic move type | Defer | What is the source and order policy for Ate abilities, Liquid Voice, Normalize, and other type conversion effects under Shared Power? |
| Run Away/trapping | Defer | Do donated trapping abilities contribute to escape prevention, does donated Shadow Tag satisfy the self-exemption, and how is the trapping source displayed/recorded? |
| Player preview | Defer | Should the preview use live active abilities, selected `DamageContext`, or a separate prediction model for dynamic type and immunity effects? |
| AI damage usability | Defer | Should AI damage helpers use the same selected `DamageContext` contract as live damage or continue using active-aware AI wrappers for current pool prediction? |
...
| --- | --- | --- |
| `CanAbilityBlockMove` | Main live blocker ownership is implemented; future changes should preserve selected blocker attribution. | Who is the blocker and popup owner for any newly added ally or pooled blocker branch? |
...
| `AccuracyCheck` | Main live accuracy membership is implemented; special target mutation remains a helper-design question. | Should future helper calls receive explicit target state instead of globals? |
...
| Pooled priority/order | Pooled Prankster, Triage, and Gale Wings priority modifiers are covered; within-priority order effects remain clarify-only. | Add separate Stall/Mycelium Might/Quick Draw/Custap-style tests only if order effects are migrated. |
| Partner auras/blockers | Battery, Power Spot, Steely Spirit, Flower Gift, Friend Guard, Victory Star, Magic Bounce, and priority blockers have focused coverage across implemented buckets. | Add cases only when new partner aura/blocker branches are migrated. |
...
| Recoil/item backlash | Rocky Helmet, Jaboca/Rowap, crash/recoil, holder damage, and shared Magic Guard variants are covered by focused buckets. | Add cases only for newly migrated recoil/item branches. |
...
| AI predictions | Shared Power AI prediction coverage now includes the migrated active-ability scoring and switching buckets. | Add cases for future AI prediction migrations after their live contracts are decided. |
| Shared Power off | Recent migrated buckets include disabled/off-path checks when practical. | Keep adding disabled bench-inert checks for future migrations when the test harness can express the split cleanly. |

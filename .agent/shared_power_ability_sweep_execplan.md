...
- [x] (2026-06-08T00:08Z) Selected the AI Stockpile Contrary prediction bucket, scoped to active attacker Contrary in the Stockpile score guard.
...
- [x] (2026-06-07T06:01Z) Selected the AI Fell Stinger Contrary prediction bucket, scoped to active attacker Contrary in the KO bonus score guard.
- [x] (2026-06-07T06:09Z) Implemented the AI Fell Stinger Contrary prediction bucket and added focused Shared Power enabled/off score coverage.
...
- [x] (2026-06-07T06:30Z) Selected the AI target additional-effect Contrary prediction bucket, scoped to active target Contrary for target stat-lowering additional effects.
- [x] (2026-06-07T06:38Z) Implemented the AI target additional-effect Contrary prediction bucket and added focused Shared Power enabled/off score coverage.
...
- Observation: AI Stockpile scoring has a narrow native Contrary guard even though live stat-change behavior already uses active Contrary membership.
  Evidence: `EFFECT_STOCKPILE` in `src/battle_ai_main.c` breaks only on native `aiData->abilities[battlerAtk] == ABILITY_CONTRARY`, while live `CompareStat` and stat-change buffering use `HasActiveAbility(..., ABILITY_CONTRARY)`.

- Observation: The Stockpile score split is stable with active Contrary lowering Stockpile to default and Shared Power off leaving the setup score above default.
  Evidence: `TESTS="Shared Power AI: pooled Contrary lowers Stockpile score"` and `TESTS="Shared Power off: partner Contrary does not lower Stockpile score"` both passed 1/1.
...
- Observation: AI Fell Stinger scoring has a narrow native Contrary guard in its KO bonus path.
  Evidence: `EFFECT_FELL_STINGER` in `src/battle_ai_main.c` checks native `aiData->abilities[battlerAtk] != ABILITY_CONTRARY`, while live stat-change behavior routes Contrary through active membership.

- Observation: The Fell Stinger score split is stable with active Contrary suppressing the Fell Stinger KO bonus while leaving the ordinary fainting score, and Shared Power off preserving the higher KO bonus score.
  Evidence: `TESTS="Shared Power AI: pooled Contrary prevents Fell Stinger KO bonus"` and `TESTS="Shared Power off: partner Contrary does not prevent Fell Stinger KO bonus"` both passed 1/1. The enabled path scored `AI_SCORE_DEFAULT + 5`; the off-path was greater than that reduced score.

- Observation: The AI Fell Stinger Contrary bucket passed the broader Shared Power AI regression filter.
...
- Observation: AI target additional-effect scoring has narrow native Contrary guards before scoring target stat-lowering move effects.
  Evidence: The target-effect path in `AI_CalcMoveEffectScore` checks native `aiData->abilities[battlerDef] != ABILITY_CONTRARY` for stat-lowering additional effects, while live stat-change behavior routes Contrary through active membership.

- Observation: The target additional-effect score split is stable with active Contrary reducing Acid Spray's target Sp. Def drop score and Shared Power off preserving the higher normal stat-drop score.
  Evidence: `TESTS="Shared Power AI: pooled Contrary lowers target stat-drop additional effect score"` and `TESTS="Shared Power off: partner Contrary does not lower target stat-drop additional effect score"` both passed 1/1. The enabled path scored `AI_SCORE_DEFAULT + 1`; the off-path was greater than that reduced score.

- Observation: The AI target additional-effect Contrary bucket passed the broader Shared Power AI regression filter.

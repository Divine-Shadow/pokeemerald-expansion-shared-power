...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    u32 abilityAtk = aiData->abilities[battlerAtk];
    u32 abilityDef = aiData->abilities[battlerDef];
    s32 atkPriority = AI_GetBattleMovePriority(battlerAtk, move);

    SetTypeBeforeUsingMove(move, battlerAtk);
    moveType = GetBattleMoveType(move);

    if (IsPowderMove(move) && !AI_IsAffectedByPowder(battlerDef))
    ...
    bool32 ignoreDefAbilities = DoesBattlerIgnoreAbilityChecks(battlerAtk, abilityAtk, move);
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        if (!ignoreDefAbilities)
        {
            if (AI_CanAbilityBlockMove(battlerAtk, battlerDef, move, AI_CHECK))
                RETURN_SCORE_MINUS(20);

            if (AI_CanAbilityAbsorbMove(battlerAtk, battlerDef, move, moveType, AI_CHECK))
                RETURN_SCORE_MINUS(20);
        }
        ...
        if (B_PRANKSTER_DARK_TYPES >= GEN_7 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_DARK)
          && AI_HasActiveAbility(battlerAtk, ABILITY_PRANKSTER) && IsBattleMoveStatus(move)
...
// the following checks apply to any target (including user)
    ...
    switch (moveEffect)
    {
        case EFFECT_HIT: // only applies to Vital Throw
            if (AI_GetBattleMovePriority(battlerAtk, move) < 0 && AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY) && aiData->hpPercents[battlerAtk] < 40)
        ...
        case EFFECT_ROTOTILLER:
            if (hasPartner)
            {
                if (!(IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GRASS)
                  ...
                  && !AI_HasActiveAbility(BATTLE_PARTNER(battlerAtk), ABILITY_CONTRARY)
            ...
            break;
        case EFFECT_GEAR_UP:
            if (AI_HasActiveAbility(battlerAtk, ABILITY_PLUS) || AI_HasActiveAbility(battlerAtk, ABILITY_MINUS))
            {
            ...
            if (hasPartner)
            {
                if (AI_HasActiveAbility(BATTLE_PARTNER(battlerAtk), ABILITY_PLUS) || AI_HasActiveAbility(BATTLE_PARTNER(battlerAtk), ABILITY_MINUS))
                {
                ...
                else if (!AI_HasActiveAbility(battlerAtk, ABILITY_PLUS) && !AI_HasActiveAbility(battlerAtk, ABILITY_MINUS))
                {
    ...
    // other
        ...
        case EFFECT_SUBSTITUTE:
            if (gBattleMons[battlerAtk].volatiles.substitute || AI_HasActiveAbility(battlerDef, ABILITY_INFILTRATOR))
                ADJUST_SCORE(-8);
        ...
        case EFFECT_PERISH_SONG:
            ...
            {
                if (CountUsablePartyMons(battlerAtk) == 0
                  && !AI_HasActiveAbility(battlerAtk, ABILITY_SOUNDPROOF)
                  && !AI_HasActiveAbility(BATTLE_PARTNER(battlerAtk), ABILITY_SOUNDPROOF)
                  && CountUsablePartyMons(FOE(battlerAtk)) >= 1)
                {
                    ADJUST_SCORE(-10); //Don't wipe your team if you're going to lose
                }
                else if ((!IsBattlerAlive(FOE(battlerAtk)) || AI_HasActiveAbility(FOE(battlerAtk), ABILITY_SOUNDPROOF)
                  || gBattleMons[FOE(battlerAtk)].volatiles.perishSong)
                  && (!IsBattlerAlive(BATTLE_PARTNER(FOE(battlerAtk))) || AI_HasActiveAbility(BATTLE_PARTNER(FOE(battlerAtk)), ABILITY_SOUNDPROOF)
            ...
            {
                if (CountUsablePartyMons(battlerAtk) == 0 && !AI_HasActiveAbility(battlerAtk, ABILITY_SOUNDPROOF)
                  && CountUsablePartyMons(battlerDef) >= 1)
                    ADJUST_SCORE(-10);

                if (gBattleMons[FOE(battlerAtk)].volatiles.perishSong || AI_HasActiveAbility(FOE(battlerAtk), ABILITY_SOUNDPROOF))
                    ADJUST_SCORE(-10);
            }
            break;
        case EFFECT_SANDSTORM:
        ...
        case EFFECT_MAGNITUDE:
            if (AI_HasActiveAbility(battlerDef, ABILITY_LEVITATE))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_CORROSIVE_GAS:
            if (AI_HasActiveAbility(battlerDef, ABILITY_STICKY_HOLD))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_INGRAIN:
        ...
        case EFFECT_WATER_SPORT:
            ...
            break;
        case EFFECT_ABSORB:
            if (AI_HasActiveAbility(battlerDef, ABILITY_LIQUID_OOZE))
                ADJUST_SCORE(-6);
            break;
        ...
        case EFFECT_RECOIL_IF_MISS:
            if (!AI_HasActiveAbility(battlerAtk, ABILITY_MAGIC_GUARD) && gAiLogicData->moveAccuracy[battlerAtk][battlerDef][gAiThinkingStruct->movesetIndex] < 75
        ...
        case EFFECT_DEFOG:
            ...
            if (gBattleMons[battlerDef].statStages[STAT_EVASION] == MIN_STAT_STAGE
              || (AI_HasActiveAbility(battlerDef, ABILITY_CONTRARY) && !IsTargetingPartner(battlerAtk, battlerDef))) // don't want to raise target stats unless its your partner
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_SPICY_EXTRACT:
            if (battlerAtk != BATTLE_PARTNER(battlerDef)
             && (HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL)
              ...
              || AI_HasActiveAbility(battlerDef, ABILITY_GOOD_AS_GOLD)
              || aiData->holdEffects[battlerDef] == HOLD_EFFECT_CLEAR_AMULET))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_UPPER_HAND:
            {
                u32 defPrio = AI_GetBattleMovePriority(battlerDef, predictedMove);
                if (predictedMove == MOVE_NONE
...
static s32 AI_TryToFaint(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
        ...
        && effect != EFFECT_EXPLOSION && effect != EFFECT_MISTY_EXPLOSION)
    {
        if (AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY))
            ...
            && AI_GetBattleMovePriority(battlerAtk, move) > 0)
    {
...
static s32 AI_DoubleBattle(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (MoveAlwaysCrits(aiData->partnerMove) && AI_HasActiveAbility(battlerAtk, ABILITY_ANGER_POINT))
    {
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_ROTOTILLER:
            ...
            {
                if (AI_HasActiveAbility(FOE(battlerAtk), ABILITY_CONTRARY))
                    ADJUST_SCORE(WEAK_EFFECT);
                else
                    ADJUST_SCORE(AWFUL_EFFECT);
            }
            ...
            {
                if (AI_HasActiveAbility(BATTLE_PARTNER(FOE(battlerAtk)), ABILITY_CONTRARY))
                    ADJUST_SCORE(WEAK_EFFECT);
                else
                    ADJUST_SCORE(AWFUL_EFFECT);
            }
    ...
    case EFFECT_POPULATION_BOMB:
        if (AI_MoveMakesContact(aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk], move)
          && !AI_HasActiveAbility(battlerAtk, ABILITY_MAGIC_GUARD)
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || AI_HasActiveAbility(battlerAtk, ABILITY_SHED_SKIN)
              || AI_HasActiveAbility(battlerAtk, ABILITY_EARLY_BIRD)
              || (AI_GetWeather() & B_WEATHER_RAIN && gWishFutureKnock.weatherDuration != 1 && AI_HasActiveAbility(battlerAtk, ABILITY_HYDRATION) && aiData->holdEffects[battlerAtk] != HOLD_EFFECT_UTILITY_UMBRELLA))
                ADJUST_SCORE(GOOD_EFFECT);
        }
        break;
    case EFFECT_OHKO:
    ...
    case EFFECT_LASER_FOCUS:
        if (AI_HasActiveAbility(battlerAtk, ABILITY_SUPER_LUCK)
         || AI_HasActiveAbility(battlerAtk, ABILITY_SNIPER)
    ...
    case EFFECT_LEECH_SEED:
        if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)
          ...
          || AI_HasActiveAbility(battlerDef, ABILITY_MAGIC_GUARD))
            break;
        ADJUST_SCORE(GOOD_EFFECT);
        if (!HasDamagingMove(battlerDef)
    ...
    case EFFECT_PROTECT:
        ...
        case PROTECT_WIDE_GUARD:
            ...
            {
                if (!AI_HasActiveAbility(battlerAtk, ABILITY_TELEPATHY))
                  ADJUST_SCORE(ProtectChecks(battlerAtk, battlerDef, move, predictedMove));
            }
            break;
        case PROTECT_CRAFTY_SHIELD:
    ...
    case EFFECT_TOXIC_SPIKES:
        ...
        break;
    case EFFECT_FORESIGHT:
        if (AI_HasActiveAbility(battlerAtk, ABILITY_SCRAPPY) || AI_HasActiveAbility(battlerAtk, ABILITY_MINDS_EYE))
            break;
    ...
    case EFFECT_FELL_STINGER:
        if (gBattleMons[battlerAtk].statStages[STAT_ATK] < MAX_STAT_STAGE
        && !AI_HasActiveAbility(battlerAtk, ABILITY_CONTRARY)
    ...
    case EFFECT_FOLLOW_ME:
        if (hasPartner
          ...
          && !IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef])
          && (!IsPowderMove(move) || AI_IsAffectedByPowder(battlerDef)))
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_BLACK_SLUDGE:
            if (!IS_BATTLER_OF_TYPE(battlerDef, TYPE_POISON) && !AI_HasActiveAbility(battlerDef, ABILITY_MAGIC_GUARD))
                ADJUST_SCORE(DECENT_EFFECT);
            break;
        ...
        case HOLD_EFFECT_UTILITY_UMBRELLA:
            if (!AI_HasActiveAbility(battlerAtk, ABILITY_SOLAR_POWER) && !AI_HasActiveAbility(battlerAtk, ABILITY_DRY_SKIN))
            {
        ...
        default:
            if (GetMoveEffect(move) != EFFECT_BESTOW && aiData->items[battlerAtk] == ITEM_NONE && aiData->items[battlerDef] != ITEM_NONE)
            {
                ...
                case HOLD_EFFECT_FLAME_ORB:
                    ...
                    break;
                case HOLD_EFFECT_BLACK_SLUDGE:
                    if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_POISON) || AI_HasActiveAbility(battlerAtk, ABILITY_MAGIC_GUARD))
                        ADJUST_SCORE(DECENT_EFFECT);
                    break;
                case HOLD_EFFECT_IRON_BALL:
    ...
    case EFFECT_RECYCLE:
        ...
        if (IsRecycleEncouragedItem(GetUsedHeldItem(battlerAtk)))
            ADJUST_SCORE(WEAK_EFFECT);
        if (AI_HasActiveAbility(battlerAtk, ABILITY_RIPEN))
        {
    ...
    case EFFECT_STEAL_ITEM:
        {
            ...
            if (canSteal && aiData->items[battlerAtk] == ITEM_NONE
             ...
             && !AI_HasActiveAbility(battlerDef, ABILITY_STICKY_HOLD))
            {
                switch (aiData->holdEffects[battlerDef])
                {
    ...
    for (i = 0; i < additionalEffectCount; i++)
    {
        ...
        else // consider move effects that hinder the target
        {
            ...
            case MOVE_EFFECT_BUG_BITE:   // And pluck
                if (gBattleMons[battlerDef].volatiles.substitute || AI_HasActiveAbility(battlerDef, ABILITY_STICKY_HOLD))
                    break;
...
static bool32 AI_TargetHealsFromAbsorbAbility(u32 battlerDef, u32 moveType)
{
    ...
    {
    case TYPE_ELECTRIC:
        return AI_HasActiveAbility(battlerDef, ABILITY_VOLT_ABSORB);
    case TYPE_GROUND:
        return AI_HasActiveAbility(battlerDef, ABILITY_EARTH_EATER);
    case TYPE_WATER:
        return AI_HasActiveAbility(battlerDef, ABILITY_DRY_SKIN)
            || AI_HasActiveAbility(battlerDef, ABILITY_WATER_ABSORB);
    default:
        return FALSE;
    }
}
...
static s32 AI_HPAware(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         || AI_TargetHealsFromAbsorbAbility(battlerDef, moveType))
        {
            if (gBattleMons[battlerDef].volatiles.healBlock)
                return 0;

...
- `struct SharedPowerPoolAddResult SharedPower_AddToPool(u8 trainerIdx, u16 ability, u8 sourcePartyIndex)` — insert (dedup), returns `{ addedUnique, addedAll }` and always records a source entry even when duplicates exist.

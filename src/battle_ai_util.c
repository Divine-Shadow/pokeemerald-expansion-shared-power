...
bool32 CanEndureHit(u32 battler, u32 battlerTarget, u32 move)
{
    ...
    {
        if (B_STURDY >= GEN_5 && AI_HasActiveAbility(battlerTarget, ABILITY_STURDY))
            return TRUE;
...
enum ItemHoldEffect AI_DecideHoldEffectForTurn(u32 battlerId)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
        return HOLD_EFFECT_NONE;
    if (AI_HasActiveAbility(battlerId, ABILITY_KLUTZ) && !gBattleMons[battlerId].volatiles.gastroAcid)
        return HOLD_EFFECT_NONE;
...
void IncreaseConfusionScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
      ...
      && gAiLogicData->holdEffects[battlerDef] != HOLD_EFFECT_CURE_STATUS)
    {
        if (gBattleMons[battlerDef].status1 & STATUS1_PARALYSIS
          || gBattleMons[battlerDef].volatiles.infatuation
          || (AI_HasActiveAbility(battlerAtk, ABILITY_SERENE_GRACE) && HasMoveWithMoveEffectExcept(battlerAtk, MOVE_EFFECT_FLINCH, EFFECT_FIRST_TURN_ONLY)))
            ADJUST_SCORE_PTR(GOOD_EFFECT);
        else
            ADJUST_SCORE_PTR(DECENT_EFFECT);
    }
